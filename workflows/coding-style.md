...
Tables **must** have columns aligned with padding. Never use compressed `|---|---|` tables.

```markdown
<!-- ❌ Wrong — compressed, hard to read in raw markdown -->
| Метрика   | До   | После |
| --------- | ---- | ----- |
| Конверсия | 2.1% | 3.8%  |

<!-- ✅ Correct — aligned, easy to scan -->
| Метрика   | До   | После |
| --------- | ---- | ----- |
| Конверсия | 2.1% | 3.8%  |
```
...
**Rules:**
- Separator row (`---`) must extend to match the widest cell content in each column
- Cell content should have consistent padding with spaces
- Every table cell should have at least one leading and trailing space
...
1. All tables must have **aligned columns with padding** — never use compressed `|---|---|` separators.
